...
We wish to study numerical algorithms for solving partial differential equations, using different finite difference schemes. The partial differential equation we will be employing, is a simplified case of the diffusion equation, normally defined as
...
The diffusion equation is used to describe so-called Brownian motion, where large systems of random microscopic particles is described by it's macroscopic characteristics. The quantity $u(\textbf{r},t)$ may be physically interpreted as, among other things, the materials density, energy consentration, or temperature gradient, at a given point in time and space.
\\\\
...
\end{equation}
Since we are not looking at a specific physical interpretation of the diffusion equation, D is left as an unknown constant. We will further simplify the equation by scaling our variables such that they become dimmentionless, and D disappears. This leaves us with the partial differential equation we will study.
\begin{equation}
\nabla^2 u(\textbf{r},t) = \frac{\pu(\textbf{r},t)}{\pt}
\end{equation}
Writing out the Laplace-operator, this becomes \ref{eqn:PDE} in one spatial dimension, and \ref{eqn:PDE2} in two spatial dimensions, where $x$, $y$, and $t$ now are dimmentionless variables.
...
The spatial derivatives on the left side will be approximated with central difference, and the time-derivative on the right using Forward Euler, Backward Euler, and the Crank-Nicolson scheme. \textbf{TODO: Write some more about how we wish to implement the differential equation numerically}
\\\\

\section{Method and Idea}
\subsection{Discretization and notation}\label{sec:disc}
The spatial dimensions will be discretized as points
\begin{equation} (x_i), \quad i \in [0,n+1] \end{equation}
in the one dimensional case, and points 
...
\textbf{TODO}: Deriving crank-nicolson scheme more thoroughly, among other tings: showing the truncations error.

While the Forward and Backward Euler methods rely on taking a step forwards and backwards in time to approximate the derivative, the Crank-Nicolson scheme simply takes the average of the two:
\begin{equation}
\frac{u_i^l-u_i^{l-1}}{\dt} = \frac{1}{2}\left( \frac{u_{i+1}^l - 2u_i^l + u_{i-1}^l}{\dx^2} + \frac{u_{i+1}^{l-1} - 2u_i^{l-1} + u_{i-1}^{l-1}}{\dx^2}\right)
\end{equation}
After again defining $\alpha = \frac{\dt}{\dx^2}$, we can seperate the two timesteps $t_l$ and $t_{l-1}$, giving us the implicit Crank-Nicolson scheme.
\begin{equation}
...
Since all methods build on the central difference scheme, the trucation error of the spatial derivative runs as $\dx^2$ for all schemes. From deriving the three methods in sections ( \textbf{TODO}:refrencing ) we know that the Euler methods has a truncation error running as $\dt$, while it is $\dt^2$ for the Crank-Nicolson scheme.
\\\\
...
We will now expand our implementations to two spatial dimmentions, resulting in the partial differential equation, $u_{xx} + u_{yy} = u_t$, or, written out:
\begin{equation}
\left(\frac{\ppu(x,y,t)}{\px^2} + \frac{\ppu(x,y,t)}{\py^2}\right) = \frac{\pu(x,y,t)}{\pt}
\label{eq:diffusion_2d}
\end{equation}
The central difference scheme derived for one dimension in \ref{sec:cent} can easially be expanded to two spatial dimensions, giving the left side of the PDE:
\begin{equation}
u_{xx} + u_{yy} = \frac{u_{i+1,j}^l - 2u_i^l+u_{i-1,j}^l}{\dx^2} + \frac{u_{i,j+1}^l - 2u_i^l+u_{i,j-1}^l}{\dy^2}
\end{equation}

As we have only introduced a new spatial dimension, the schemes for approximating time derivates remains unchanged. We expand the notation of the Forward Euler method to two dimensions:
\begin{equation}
