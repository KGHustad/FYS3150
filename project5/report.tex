...
\textbf{TODO}: Deriving crank-nicolson scheme more thoroughly?
...
\\ \hline \\
Forward Euler	& $\dt, \dx^2$		& only $\dt \le \frac{1}{2}\dx^2$ \\
\\ \hline \\
Backward Euler	& $\dt, \dx^2$		& all $\dt$ and $\dx$ \\
\\ \hline \\
Crank-Nicolson	& $\dt^2\dx$		& all $\dt$ and $\dx$ \\
\\ \hline \\
\end{tabular}
\end{center}
\caption{Truncation error and stability requirements for the three solutions}
\label{table:error}
\end{table}

\subsubsection{Stability of chosen methods}
