...
We clearly see the impact of the decreased $\dt$ value from 0.1 to 0.01 in \ref{fig:1d:sinus:T=0.1} and \ref{fig:1d:sinus:T=1.0}. At $\dt = 0.01$ it's hard to tell the difference.
\begin{figure}[H]
...
The Crank-Nicolson plot in \vref{fig:sinus:trunc:dt:cn} is identical to \vref{fig:sinus:trunc:dx}, as expected, since the error for Crank-Nicolson runs as $\dt^2$ and $\dx^2$.
...
If we keep $\dx$ constant and increase $\alpha$ as in \ref{fig:sinus:stability}, we see that Forward Euler is unstable for $\alpha > 0.5$.
...
Picking one of the methods, the Crank-Nicolson, we study how it converges (we don't have an analytical solution here, but we know that it should converge on a linear solution).
\begin{figure}[H]
...
\subsection{Speedup from parallelization}
\begin{table}[H]
\begin{center}
\begin{tabular}{rrr}
\hline
