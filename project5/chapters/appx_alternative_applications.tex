...
Obviously, the data would also have \emph{appeared} to be smoother if we had reduced the number of points along the x-axis, but that would limit the precision to which we could find the maximum of the function, so we wanted to avoid that.
...
\caption{Specific heat with $\Delta t = 0.001$ computed from the sweeps in $(1 \cdot 10^{6}, 2 \cdot 10^{6}]$}
...
\caption{Specific heat with $\Delta t = 0.001$ computed from the sweeps in $(1 \cdot 10^{6},  10^{7}]$}
